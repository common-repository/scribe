...
?>
<form method="post">
	<h3><?php esc_html_e( 'Contextual Analysis' , 'scribe'); ?></h3>
	<p>
				...
				<?php if( $keywordDescription['Type'] == 'Primary Keywords') { ?>
					...
					<td>
						...
						if( count( $primaryKeywords ) == 0 ) {
							...
							printf(esc_html__('The term %s is emphasized within your content and is considered a Primary Keyword.' , 'scribe'), '<strong>' . esc_html( $primaryKeywords[0] ) . '</strong>' );
						} elseif(count($primaryKeywords)==2) {
							printf(esc_html__('The terms %s and %s are emphasized within your content and are considered Primary Keywords.', 'scribe'),'<strong>' . esc_html( $primaryKeywords[0] ) . '</strong>','<strong>' . esc_html( $primaryKeywords[1] ) . '</strong>');
						} else {
							$primaryKeywords = array_map( 'esc_html', $primaryKeywords );
							$last = array_pop($primaryKeywords);
							printf(esc_html__('The terms %s, and %s are emphasized within your content and are considered Primary Keywords.', 'scribe'),'<strong>' . implode('</strong>, <strong>', $primaryKeywords) . '</strong>',"<strong>{$last}</strong>");
						}
						?>
					</td>
				<?php } else { ?>
	...
	<h3><?php esc_html_e( 'Keyword Analysis' , 'scribe'); ?></h3>
	<p>
	...
	<table class="widefat" style="width: 99%;">
		<thead>
			<tr>
				<th scope="col"><?php esc_html_e( 'Keywords' , 'scribe'); ?></th>
				<th scope="col"><?php esc_html_e( 'Rank' , 'scribe'); ?></th>
				<th scope="col"><?php esc_html_e( 'Prominence' , 'scribe'); ?></th>
				<th scope="col"><?php esc_html_e( 'Frequency' , 'scribe'); ?></th>
				<th scope="col"><?php esc_html_e( 'Density' , 'scribe'); ?></th>
			</tr>
		</thead>
		<tbody>
			<?php if( !is_array($keywordInfo['Keywords']['Keyword'] ) ) { ?>
				<tr class="normal">
					<td colspan="6" style="text-align: center;"><?php esc_html_e( 'No keywords found.' , 'scribe'); ?></td>
				</tr>
			<?php } else { 
