	...
	class Ecordia {
		...
		function addAdminInterfaceItems() {
			...
			if(current_user_can($permission)) {
				...
				$title = esc_html__('Scribe v3 Score', 'scribe');
		...
		function thickboxTabs($tabs) {
			$pages = array(
				'ecordia-score' => esc_html__('SEO Score', 'scribe'), 
				'ecordia-keyword-analysis' => esc_html__('Keyword Analysis', 'scribe'), 
				'ecordia-change-keywords' => esc_html__('Change Keywords', 'scribe'), 
				'ecordia-keyword-alternates' => esc_html__('Alternate Keywords', 'scribe'), 
				'ecordia-tags' => esc_html__('Tags', 'scribe'), 
				'ecordia-serp' => esc_html__('SERP', 'scribe'), 
				'ecordia-seo-best-practices' => esc_html__('SEO Best Practices', 'scribe')
			);
			if( defined( 'ECORDIA_DEBUG' ) && ECORDIA_DEBUG ) {
				$pages['ecordia-debug'] = esc_html__( 'Debug Info' , 'scribe');
			}
			return $pages;
		}
		function linkBuildingThickboxTabs($tabs) {
			return array(
				'ecordia-link-building-external' => esc_html__('External Links', 'scribe'), 
				'ecordia-link-building-internal' => esc_html__('Internal Links', 'scribe'), 
				'ecordia-link-building-social' => esc_html__('Social Media', 'scribe')
			);
		}
		...
		function getAutomaticDependencyNiceName($automatic) {
			switch( $automatic ) {
				case 'aioseo':
					$currently = esc_html__( 'All in One SEO Pack Plugin' , 'scribe');
					break;
				case 'headwa':
					$currently = esc_html__( 'Headway Theme' , 'scribe');
					break;
				case 'hybrid':
					$currently = esc_html__( 'Hybrid Theme' , 'scribe');
					break;
				case 'genesis':
					$currently = esc_html__( 'Genesis Theme' , 'scribe');
					break;
				case 'thesis':
					$currently = esc_html__( 'Thesis Theme' , 'scribe');
					break;
				case 'fvaioseo':
					$currently = esc_html__( 'FV All in One SEO Pack' , 'scribe');
					break;
				case 'woothemes':
					$currently = esc_html__( 'WooThemes' , 'scribe');
					break;
				default:
					$currently = sprintf( '<span class="ecordia-error">%1$s</span>.  %2$s <a href="http://scribeseo.com/compatibility" target="_blank">http://scribeseo.com/compatibility</a>.', esc_html__( 'unable to detected theme/plugin', 'scribe' ), esc_html__( 'Please select the "Choose the SEO tool" option below. To see a list of compatible plugins and themes go to ' , 'scribe' ) );
					break;
			}
			return $currently;
		}
		...
		function getUserInfo($live = false) {
			...
			if ( $live && empty( $settings['api-key'] ) ) {
				...
				return new WP_Error(-1, esc_html__('You must set an API key.', 'scribe'));
			...
			if ( $live ) {
				...
				return new WP_Error(-100, esc_html__('Fetching Information...', 'scribe'));
