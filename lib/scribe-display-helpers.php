...
function scribe_get_keyword_analysis_score_text($score) {
	return apply_filters( 'scribe_get_keyword_analysis_score_text', 
		scribe_get_content_for_letter_score(
			$score,
			__( 'Congratulations, this term strikes a good balance between search optimization and copywriting best practices.', 'scribe' ),
			__( 'You\'re close! This keyword requires a bit more work to reach the correct balance between search optimization and copywriting best practices.', 'scribe' ),
			__( 'You need to integrate this keyword more closely into your copy to achieve a better score and make it more relevant.', 'scribe' ),
			__( 'Please reevaluate your use of this keyword to improve its ranking.', 'scribe' )
		)
	);
}
...
function scribe_get_content_analyis_site_score_text( $score, $keywords ) {
	...
	if ( $score < 50 ) {
		...
		$text = __( 'While terms like <strong>%1$s</strong> and <strong>%2$s</strong> appear in your page, your site does not use these terms frequently. Consider adding more content to your site for these terms. %3$s', 'scribe' );
		$improve_text = '';

	} else {

		$text = __( 'This page is a good match for your site for terms like <strong>%1$s</strong> and <strong>%2$s</strong>. %3$s', 'scribe' );
		$improve_text = __( 'To improve your Site Score, consider adding more content related to these terms.', 'scribe' );
	...
	reset( $kws );
	$keyword_list = implode( __( '</strong> and <strong>', 'scribe' ), $primary_keywords );

	if ( ! empty( $primary_keywords ) ) {

		if ( $score < 70 && key( $kws ) < 5 )
			$text = sprintf( __( 'While the terms <strong>%s</strong> are used in your page, they are not used frequently enough on your site. Consider adding more content to your site for these terms.', 'scribe' ), $keyword_list );
		elseif ( ! $primary )
			$improve_text = sprintf( __( ' To improve your Site Score, consider adding more content related to <strong>%s</strong>, so that this page better aligns with your site.', 'scribe' ), $keyword_list );
...
function scribe_get_content_analysis_doc_score_text( $score, $keywords ) {
	...
	if ( $score < 100 )
		return __( 'Your page needs some improvement. Review the suggestions under Improve Page Structure to raise your score.', 'scribe' );
	...
	if ( $score == 100 ) {
		...
		return sprintf( __( 'Your page does a good job of meeting the best practices of SEO Copywriting for the term <strong>%s</strong>.', 'scribe' ), implode( __( '</strong> and <strong>', 'scribe' ), $primary_keywords ) );
	}

	return __( 'Your page does a good job of meeting the best practices of SEO Copywriting.', 'scribe' );
...
function scribe_get_overall_score_description($score) {
	return apply_filters( 'scribe_get_overall_score_description',
		scribe_get_content_for_score(
			$score,
			__( 'It will not be difficult for you to rank on a search engine for this keyword.', 'scribe' ),
			__( 'This term will be somewhat difficult for you to rank for on a search engine.', 'scribe' ),
			'',
			'overall'
		)
	);
...
function scribe_get_content_score_description($score) {
	return apply_filters( 'scribe_get_content_score_description',
		scribe_get_content_for_score(
			$score,
			__( 'Great! You have an awesome score!', 'scribe'),
			__( 'Consider adding more content to your site for this term.', 'scribe' ),
			__( 'Your site exceeds the amount of content for this term for sites that are ranking.', 'scribe' )
		)
	);
}
...
function scribe_get_link_score_description($score) {
	return apply_filters( 'scribe_get_link_score_description',
		scribe_get_content_for_score(
			$score,
			__( 'Great! You have an awesome score!', 'scribe' ),
			__( 'You will need to obtain more links to your site for this term.', 'scribe' ),
			__( 'Your site exceeds the number of links for sites that currently rank for this term.', 'scribe' )
		)
	);
}
...
function scribe_get_domain_authority_score_description($score) {
	return apply_filters( 'scribe_get_domain_authority_score_description',
		scribe_get_content_for_score(
			$score,
			__( 'Great! You have an awesome score!', 'scribe' ),
			__( 'You should obtain more links for this term from sites that have strong authority.', 'scribe' ),
			__( 'Your site’s authority exceeds the authority of most sites that rank for this term.', 'scribe' )
		)
	);
}
...
function scribe_get_facebook_likes_score_description($score) {
	return apply_filters('scribe_get_facebook_likes_score_description',
		scribe_get_content_for_score(
			$score,
			__( 'Great! You have an awesome score!', 'scribe' ),
			__( 'You should have more people sharing your content online in order to rank well for this term.', 'scribe' ),
			__( 'Your site’s content for this term is highly socialized.', 'scribe' )
		)
	);
}
