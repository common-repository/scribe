...
abstract class Scribe_Admin {
	...
	public function create( $page_id = '', $menu_ops = array(), $page_ops = array(), $settings_field = '', $default_settings = array() ) {
		...
		$this->page_ops = wp_parse_args( $this->page_ops, array(
			'save_button_text'  => __( 'Save Settings', 'scribe' ),
			'reset_button_text' => __( 'Reset Settings', 'scribe' ),
			'saved_notice_text' => __( 'Settings saved.', 'scribe' ),
			'reset_notice_text' => __( 'Settings reset.', 'scribe' )
		) );
		...
		if ( isset( $this->menu_ops['submenu'] ) && ( isset( $this->menu_ops['main_menu'] ) || isset( $this->menu_ops['first_submenu'] ) ) )
			wp_die( sprintf( esc_html__( 'You cannot use %s to create two menus in the same subclass. Please use separate subclasses for each menu.', 'scribe' ), 'Scribe_Admin' ) );
	...
	public function admin() {
		...
		<form method="post" action="options.php">
			...
			<h2>
				...
				submit_button( esc_attr( $this->page_ops['reset_button_text'] ), 'button-secondary accesspress-h2-button', $this->get_field_name( 'reset' ), false, array( 'onclick' => 'return accesspress_confirm(\'' . esc_js( __( 'Are you sure you want to reset?', 'scribe' ) ) . '\');' ) );
				?>
			</h2>
			...
			<div class="bottom-buttons">
				...
				submit_button( esc_attr( $this->page_ops['reset_button_text'] ), 'secondary', $this->get_field_name( 'reset' ), false, array( 'onclick' => 'return accesspress_confirm(\'' . esc_js( __( 'Are you sure you want to reset?', 'scribe' ) ) . '\');' ) );
				?>
			</div>
		</form>
		</div>
	...
	public function admin() {
		...
		<form method="post" action="options.php">
			...
			<h2>
				...
				echo esc_html( get_admin_page_title() );
				submit_button( esc_attr( $this->page_ops['save_button_text'] ), 'button-primary accesspress-h2-button', 'submit', false );
				submit_button( esc_attr( $this->page_ops['reset_button_text'] ), 'button-highlighted accesspress-h2-button', $this->get_field_name( 'reset' ), false, array( 'onclick' => 'return accesspress_confirm(\'' . esc_js( __( 'Are you sure you want to reset?', 'scribe' ) ) . '\');' ) );
				?>
			</h2>
