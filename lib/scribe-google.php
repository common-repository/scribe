	...
	class Scribe_Google {
		...
		public static function get_suggestions($search_term) {
			...
			if ( empty( $search_term ) )
				return new WP_Error('empty_term', __('Please provide a search term.', 'scribe'));
			...
			if ( ! empty( $xml_errors ) )
				return new WP_Error('xml_error', __('The XML from the Google Completion API could not be loaded appropriately.', 'scribe'));
