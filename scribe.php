	...
	class Scribe_SEO {
		...
		public static function ajax_handle_analyze_content() {
			...
			if(isset($data['scribe-analyze-content-nonce']) && wp_verify_nonce($data['scribe-analyze-content-nonce'], 'scribe-analyze-content')) {
				...
				$results = array('error' => true, 'error_message' => __('Invalid request.', 'scribe'));
			}
		...
		public static function ajax_handle_research_google_trends() {
			...
			$results = '';
			$content = sprintf(
				'<script type="text/javascript" src="%1$s//www.google.com/trends/embed.js?hl=en-US&q=___KEYWORD___&content=1&cid=TIMESERIES_GRAPH_0&export=5&w=500&h=350"></script>
<script type="text/javascript" src="%1$s//www.google.com/trends/embed.js?hl=en-US&q=___KEYWORD___&content=1&cid=TOP_QUERIES_0_0&export=5&w=320&h=510"></script>
<script type="text/javascript" src="%1$s//www.google.com/trends/embed.js?hl=en-US&q=___KEYWORD___&content=1&cid=RISING_QUERIES_0_0&export=5&w=320&h=520"></script>',
				is_ssl() ? 'https:' : 'http:'
			);
		...
		public static function ajax_handle_build_links() {
			...
			if(isset($data['scribe-build-links-nonce']) && wp_verify_nonce($data['scribe-build-links-nonce'], 'scribe-build-links')) {
				...
				if ( is_wp_error( $analysis ) ) {
					...
					switch( $type ) {
						case 'scr':
							...
							$link_analysis->score_description = sprintf( __( scribe_get_link_building_score_description( $link_analysis->score ), 'scribe' ), implode( __( ' and ', 'scribe' ), $keywords ) );
							break;
						case 'int':
				...
				$results = array('error' => true, 'error_message' => esc_js( __( 'Invalid request.', 'scribe' ) ) );
			}
		...
		public static function add_administrative_interface_items() {
			self::$admin_page_hooks[] = $connections = add_submenu_page(self::SETTINGS_TOP_PAGE_SLUG, __( 'Site Connections', 'scribe' ), __( 'Site Connections', 'scribe' ), 'manage_options', self::SETTINGS_SUB_PAGE_SLUG_CONNECTIONS, array(__CLASS__, 'display_connections_page'));
			self::$admin_page_hooks[] = $account = add_submenu_page(self::SETTINGS_TOP_PAGE_SLUG, __('Account', 'scribe'), __('Account', 'scribe'), 'manage_options', self::SETTINGS_SUB_PAGE_SLUG_ACCOUNT, array(__CLASS__, 'display_account_page'));

			if ( ! self::is_hosted() && ! self::is_managed() ) {

				self::$admin_page_hooks[] = $compatibility = add_submenu_page(self::SETTINGS_TOP_PAGE_SLUG, __('Compatibility', 'scribe'), __('Compatibility', 'scribe'), 'manage_options', self::SETTINGS_SUB_PAGE_SLUG_COMPATIBILITY, array(__CLASS__, 'display_compatibility_page'));
				add_action("load-{$compatibility}", array(__CLASS__, 'process_compatibility_check'));

			}

			self::$admin_page_hooks[] = $news = add_submenu_page(self::SETTINGS_TOP_PAGE_SLUG, __('SEO News', 'scribe'), __('SEO News', 'scribe'), 'manage_options', self::SETTINGS_SUB_PAGE_SLUG_NEWS, array(__CLASS__, 'display_news_page'));

			if ( ! self::is_hosted() && ! self::is_managed() ) {

				self::$admin_page_hooks[] = $support = add_submenu_page(self::SETTINGS_TOP_PAGE_SLUG, __( 'Scribe Help', 'scribe' ), __( 'Scribe Help', 'scribe' ), 'manage_options', self::SETTINGS_SUB_PAGE_SLUG_SUPPORT, array(__CLASS__, 'display_support_page'));
		...
		public static function add_meta_boxes($post) {
			add_meta_box( 'scribe-keyword-research', __( 'Scribe Keyword Research', 'scribe' ), array( __CLASS__, 'display_keyword_research_meta_box'), $post->post_type, 'side', 'high' );
			add_meta_box( 'scribe-analysis', __( 'Scribe Content Optimizer', 'scribe' ), array( __CLASS__, 'display_analysis_meta_box'), $post->post_type, 'side', 'high' );
			add_meta_box( 'scribe-link-building', __( 'Scribe Link Building', 'scribe' ), array( __CLASS__, 'display_link_building_meta_box'), $post->post_type, 'side', 'high' );
		}
		
		public static function add_scribe_columns($columns) {
			$columns['scribe-doc-score'] = __('Scribe Page Score', 'scribe');
			$columns['scribe-site-score'] = __('Scribe Site Score', 'scribe');
			$columns['scribe-keywords'] = __('Scribe Keywords', 'scribe');
			if ( self::$scribe_has_v3_data )
				$columns['scribe-v3'] = __('Scribe v3 Score', 'scribe' );
			...
			if ( empty( $content_analysis ) ) {
				switch( $column_name ) {
					...
					case 'scribe-keywords':
						esc_html_e( 'Awaiting Analysis', 'scribe' );
						break;
				}
				return;
			}
		...
		public static function add_configuration_script() {
			...
			$configuration = array(
				'content_analysis_label' => __('Content Analysis', 'scribe'),
				'keyword_research_label' => __('Keyword Research', 'scribe'),
		...
		public static function process_compatibility_check() {
			...
			if(!empty($data['send-scribe-compatibility-report-nonce']) && wp_verify_nonce($data['send-scribe-compatibility-report-nonce'], 'send-scribe-compatibility-report')) {
				...
				if($send_successful) {
					add_settings_error('general', 'settings_updated', __('Your results were successfully sent to the Copyblogger support team.', 'scribe'), 'updated');
				} else {
					add_settings_error('general', 'settings_updated', sprintf( esc_html__('There was an issue sending your report. Please copy the below information and email it directly to %s.', 'scribe'), '<a href="mailto:support@copyblogger.com">support@copyblogger.com</a>' ), 'error');
				}
		...
		public static function return_analysis_tabs($tabs) {
			$tabs = array(
				'scribe-analysis-keywords' => __('Keyword Analysis', 'scribe'),
				'scribe-analysis-document' => __('Page Analysis', 'scribe'),
				'scribe-analysis-tags' => __('Tags', 'scribe'),
				'scribe-analysis-help' => __('Help', 'scribe'),
			);
		...
		public static function return_keyword_tabs($tabs) {
			$tabs = array(
				'scribe-keyword-suggestions' => __('Suggestions', 'scribe')
			);
			
			if($_GET['tab'] == 'scribe-keyword-details') {
				$tabs['scribe-keyword-details'] = __('Details', 'scribe');
			}
		...
		public static function display_connections_page() {
			...
			add_meta_box('scribe-main-connections', __( 'Site Connections', 'scribe' ), array(__CLASS__, 'display_connections_meta_box'), 'scribe-connections', 'normal');
		...
		public static function display_account_page() {
			...
			add_meta_box('scribe-main-account', __('Account Information', 'scribe'), array(__CLASS__, 'display_account_meta_box'), 'scribe-account', 'normal');
		...
		public static function display_compatibility_page() {
			add_meta_box('scribe-information-compatibility', __('Your Information', 'scribe'), array(__CLASS__, 'display_compatibility_info_meta_box'), 'scribe-compatibility', 'normal');
			add_meta_box('scribe-results-compatibility', __('Your Install Details', 'scribe'), array(__CLASS__, 'display_compatibility_results_meta_box'), 'scribe-compatibility', 'normal');
		...
		public static function display_analysis_iframe() {
			...
			switch($tab) {
				...
				default:
					...
					// check for overlap
					$maxkwr = count( $keywords ) ? max( array_keys( $keywords ) ) : 0;
					for( $i = 0; $i < $maxkwr; $i++ ) {
						if ( ! isset( $keywords[$i] ) )
							continue;
...
<?php
		...
		private static function get_keyword_recommendations( $possessed_keys, $keyword ) {
			...
			$desired_keys[0] = array(
				'KwET' => __( 'Add the keyword to the HTML title tag of your content.', 'scribe' ),
				'KwETP' => __( 'Add the keyword to the beginning of your HTML title tag.', 'scribe' ),
				'KwEM' => __( 'Add the keyword to the META DESCRIPTION tag of your content.', 'scribe' ),
				'KwEMP' => __( 'Add the keyword to the beginning of the META DESCRIPTION tag of your content.', 'scribe' ),
				'KwED' => __( 'Reduce the number of times you use the keyword in your content.', 'scribe' ),
				'KwEH' => __( 'Include the keyword in the title attribute of your hyperlink/anchor text.', 'scribe' ),
				'KwELF' => __( 'Add this keyword more frequently to the page.', 'scribe' ),
			);
			$desired_keys[1] = array(
				'KwET' => __( 'Add the keywords to the HTML title tag of your content.', 'scribe' ),
				'KwETP' => __( 'Add the keywords to the beginning of your HTML title tag.', 'scribe' ),
				'KwEM' => __( 'Add the keywords to the META DESCRIPTION tag of your content.', 'scribe' ),
				'KwEMP' => __( 'Add the keywords to the beginning of the META DESCRIPTION tag of your content.', 'scribe' ),
				'KwED' => __( 'Reduce the number of times you use the keywords in your content.', 'scribe' ),
				'KwEH' => __( 'Include the keywords in the title attribute of your hyperlink/anchor text.', 'scribe' ),
				'KwELF' => __( 'Add these keywords more frequently to the page.', 'scribe' ),
			);
		...
		private static function get_content_recommendations($possessed_keys) {
			$desired_keys = array(
				'TitleLen' => __('The HTML title of the page contains less than three words, consider adding more words to the HTML title.', 'scribe'),
				'TitleCharLen' => __('The HTML title of the page is too long, consider shortening the title to less than 72 characters.', 'scribe'),
				'MetaCharLen' => __('The META DESCRIPTION is too long, consider shortening the description to less than 165 characters.', 'scribe'),
				'BodyLen' => __('The page is too short, consider adding more words to the page.', 'scribe'),
				'Hyper' => __('No hyperlinks were found in the page, consider adding hyperlinks to relevant terms.', 'scribe'),
				'HyperLen' => __('No hyperlinks were found in the first part of the page, consider adding a hyperlink to the first paragraph.', 'scribe'),
				'NoPKW' => __( 'No Primary Keywords were found in your content. See the information under Improve Keyword Rank to improve the identification of a keyword.', 'scribe' ),
			);
		...
		private static function build_social_activity( $keyword, $links = null ) {
			...
			if ( ( empty( $links->googlePlusActivities ) && empty( $links->twitterProfiles ) ) || ( ! is_array( $links->googlePlusActivities ) && ! is_array( $links->twitterProfiles ) ) )
				return '<p>' . esc_html__( 'No Activity', 'scribe' ). '</p>';
		...
		private static function build_gplus_comments( $keyword, $comments = null ) {
			...
			foreach( (array)$comments->comments as $comment )
				$output .= sprintf( $plus_format, esc_url( 'http://plus.google.com/u/0/' . $comment->profileId ), esc_attr( $comment->profileDisplayName ), esc_url( $comment->profileImageUrl ), wp_kses( $comment->content, self::formatting_allowedtags() ), esc_html__( 'Posted by', 'scribe' ) );
		...
		private static function send_test_results( $support ) {
			...
			$email_subject = sprintf(__('Scribe Compatibility Check from %1$s at %2$s', 'scribe'), $name, $site_name);
			...
			$email_body = __( 'Support Team,', 'scribe' ) . "\n\n" .
					sprintf( __( 'This message is the result of running the Scribe SEO Compatibility Check on %1$s located at %2$s . The user provided the name %3$s and the email address %4$s. %5$s', 'scribe' ), $site_name, $site_domain, $name, $email, $issue ) . "\n\n" .
					__( 'The test results from the compatibility checker were as follows:', 'scribe' ) . "\n\n" .
					$test_results_text . 
					__( 'Regards,', 'scribe' ) .
					__( 'Scribe SEO Plugin', 'scribe' );
